...
impl<R:io::Read, W: io::Write> Interpreter<R,W>{
    ...
    pub fn run(&mut self,src : &str) -> Result<(),Error>{
        ...
        while let Some(token) = tokens.get(self.token_pointer){
            self.step(*token,&tokens).unwrap();
            self.token_pointer += 1;
        }
        Ok(())
    }

    pub fn step(&mut self,token:char,tokens:&Vec<char>)->Result<(),Error>{
        match token{
    ...
    fn output(&mut self) -> Result<(),Error>{
        ...
        self.output.write_all(&[value])?; // valueという要素が入った配列の参照をわたしてる
        Ok(())
    }
    ...
    ->Result<usize,Error>{
        for pointer in start_pointer..tokens.len(){
            match (tokens[pointer],count) {
                (']',0) => return Ok(pointer),
                (']',_) => return self.find_close_pointer(tokens,pointer+1,count-1),
                ('[',_) => return self.find_close_pointer(tokens,pointer+1,count+1),
                _ => {}
            }
        }
    ...
    fn find_open_pointer(
        ...
        for pointer in (0..end_pointer).rev(){
            match (tokens[pointer],count){
                ('[',0) => return Ok(pointer),
                ('[',_) => return self.find_open_pointer(tokens,pointer-1,count-1),
                (']',_) => return self.find_open_pointer(tokens,pointer-1,count+1),
                _ => {}
            }
        }
